...
void filtrBFLF(cRGB* Iimage, cRGB* (&Iimage1), int H, int W)
{
	int m = 0, h = 1, w = 1;
	...
	int CX = w / 2;
	int n = 1;
	double Dp = sqrt(CX * CX + CY * CY) * 0.029; 
	for (int u = 0; u < w; u++) {		
		for (int v = 0; v < w; v++) {
...
void ttf2D(RGB* masin, cRGB* (&masout), int h1, int w1)
{
	int m = 0, h = 1, w = 1, i, j;
...
{
	char infile[100] = "original.bmp";
	char outfile[100] = "deform.bmp";
	...
	ttf2D(image, Iimage, pic_i.N, pic_i.M); //получение комплексного представления RGB
	filtrBFLF(Iimage, Iimage2, pic_i.N, pic_i.M); //использ. фильтра нч Баттерворта 
	ittf2D(Iimage2, Iimage1, pic_i.N, pic_i.M); //получаем RGB из компл. RGB
	...
	std::cout << "Elasped time: " << span << std::endl;//прошедшее время 

	
	ofstream out(outfile, std::ios::out | std::ios::binary);
	//заполняем структуру bmp-файла
	out.write((char*)&f_i, sizeof(f_info));
	out.write((char*)&pic_i, sizeof(pic_info));
	//определяем RGB-значения пикселей
	for (auto i = 0; i < pic_i.N; i++) {
		for (auto j = 0; j < pic_i.M; j++) {
			out.put((char)(Iimage1[i * pic_i.M + j].B));
			out.put((char)(Iimage1[i * pic_i.M + j].G));
			out.put((char)(Iimage1[i * pic_i.M + j].R));
		}
